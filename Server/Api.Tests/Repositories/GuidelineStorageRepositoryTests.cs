...
public class GuidelineStorageRepositoryTests
{
    private readonly IMinioClient _minioClient;
    private readonly GuidelineStorageRepository _repo;

    public GuidelineStorageRepositoryTests()
    {
        _minioClient = Substitute.For<IMinioClient>();
        _repo = new GuidelineStorageRepository(_minioClient);
    }

    [Fact]
    public async Task UploadAsync_WithExistingBucket_UploadsFile()
    {
        var bucketName = "guidelines";
        var objectKey = "id.json";
        ...
        _minioClient.PutObjectAsync(Arg.Any<PutObjectArgs>(), Arg.Any<CancellationToken>())
            .Returns(new PutObjectResponse(HttpStatusCode.OK, bucketName, new Dictionary<string, string>(), 0, objectKey));
    ...
    public async Task RemoveAsync_WhenMinioThrowsException_RethrowsAsException()
    {
        ...
        _minioClient.RemoveObjectAsync(Arg.Any<RemoveObjectArgs>(), Arg.Any<CancellationToken>())
            .Returns(x => throw new MinioException("Access denied"));
